est.we.mdls <- function (z.ts,etw,p,q=p,n,ex=0,lex=NULL,case,r,we.test=FALSE)
## notation: mixture of
...
if (p>1)
{
  ...
  for (j in 1:(p-1)) 
  {
    ...
    for (i in 1:n)
    {
      ...
      se$Phi[[j]][i,] <- summary(res.final)[[i]]$coefficients[((j-1)*n+qp*k+1):(j*n+qp*k),2]
      tvals$Phi[[j]][i,] <- summary(res.final)[[i]]$coefficients[((j-1)*n+qp*k+1):(j*n+qp*k),3]
      pvals$Phi[[j]][i,] <- summary(res.final)[[i]]$coefficients[((j-1)*n+qp*k+1):(j*n+qp*k),4]      
    }
...
if (max(q,lex)>1)
{
  ...
  for (j in 1:(max(q,lex)-1)) 
  {
    ...
    for (i in 1:n)
    {
      ...
      {
        index <- c((qp*n+1+k*(j-1)):(qp*n+k*j),((p-1)*n+q*k+ex*(j-1)+1):((p-1)*n+q*k+ex*j))
      } else if (q>lex) {
      	index <- (qp*n+1+k*(j-1)):(qp*n+k*j)
      } else {
